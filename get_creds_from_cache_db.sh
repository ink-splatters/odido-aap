...
DEFAULT_CACHE_PATH=(~/Library/Containers/*/Data/Library/Caches/nl.tmobile.mytmobile)
CACHE_PATH="${CACHE_PATH:-${DEFAULT_CACHE_PATH[0]}}"
...
BEARER=$(/usr/bin/find "$CACHE_PATH" -name 'Cache.db' \
...
USER_ID=$(/usr/bin/find "$CACHE_PATH" -name 'Cache.db' \
